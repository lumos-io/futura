...
import (
	...
	"io.lumos/futura/internal/controller"
...
func init() {
	...
	utilruntime.Must(futurav1.AddToScheme(scheme))
...
func main() {
	...
	mgr, err := ctrl.NewManager(ctrl.GetConfigOrDie(), ctrl.Options{
		...
		LeaderElectionID:       "cbf7cae3.io.lumos",
	...
	// Import "google.golang.org/grpc/credentials/insecure" at the top if not already imported
	...
	// Create ClusterOptimizationConfigReconciler reconciler with grpc client and register it and poller
	cocReconciler := controller.NewClusterOptimizationConfigReconciler(mgr.GetClient(), mgr.GetScheme(), grpcConn, p)
	if err := cocReconciler.SetupWithManager(mgr); err != nil {
		log.Fatalf("unable to create ClusterOptimizationConfig controller: %v", err)
	}
