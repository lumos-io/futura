...
func NewAnalyticsController(cfg *config.Configuration) (*AnalyticsController, error) {
...
func (a *AnalyticsController) Close() error {
	if a.clickhouse != nil {
...
// GET /api/analytics/events?org_id=1&cluster_id=1&limit=100&cursor=0&reverse=false
func (a *AnalyticsController) GetEvents(c *gin.Context) {
	...
	req := &pbtl.GetEventsByClusterIdRequest{
	...
	resp, err := a.getEvents(c.Request.Context(), req)
	if err != nil {
		utils.RespondError(c, 500, "GET_EVENTS_ERROR", err.Error())
		return
	}
...
func (a *AnalyticsController) getEvents(ctx context.Context, req *pbtl.GetEventsByClusterIdRequest) (*pbtl.GetEventsResponse, error) {
	log.Debug().
...
// GET /api/analytics/nodes?org_id=1&cluster_id=1
func (a *AnalyticsController) GetNodes(c *gin.Context) {
	orgID, _ := strconv.ParseUint(c.Query("org_id"), 10, 32)
	clusterID, _ := strconv.ParseInt(c.Query("cluster_id"), 10, 64)

	req := &pbtl.GetNodesByClusterIdRequest{
		OrganizationId: uint32(orgID),
		ClusterId:      clusterID,
	}

	resp, err := a.getNodes(c.Request.Context(), req)
	if err != nil {
		utils.RespondError(c, 500, "GET_NODES_ERROR", err.Error())
		return
	}
...
func (a *AnalyticsController) getNodes(ctx context.Context, req *pbtl.GetNodesByClusterIdRequest) (*pbtl.GetNodesResponse, error) {
	log.Debug().
		Uint32("org_id", req.OrganizationId).
		Int64("cluster_id", req.ClusterId).
	...
	return &pbtl.GetNodesResponse{
		Nodes: nodes,
	}, nil
}

// GetServices retrieves services for a cluster
// GET /api/analytics/services?org_id=1&cluster_id=1
func (a *AnalyticsController) GetServices(c *gin.Context) {
	orgID, _ := strconv.ParseUint(c.Query("org_id"), 10, 32)
	clusterID, _ := strconv.ParseInt(c.Query("cluster_id"), 10, 64)

	req := &pbtl.GetServicesByClusterIdRequest{
		OrganizationId: uint32(orgID),
		ClusterId:      clusterID,
	}

	resp, err := a.getServices(c.Request.Context(), req)
	if err != nil {
		utils.RespondError(c, 500, "GET_SERVICES_ERROR", err.Error())
		return
	}
...
func (a *AnalyticsController) getServices(ctx context.Context, req *pbtl.GetServicesByClusterIdRequest) (*pbtl.GetServicesResponse, error) {
	log.Debug().
		Uint32("org_id", req.OrganizationId).
		Int64("cluster_id", req.ClusterId).
	...
	var services []*pbtl.ServiceMetrics
	...
	return &pbtl.GetServicesResponse{
		Services: services,
	}, nil
}

// GetOverviewMetrics retrieves overview metrics for a cluster
// GET /api/analytics/overview?org_id=1&cluster_id=1
func (a *AnalyticsController) GetOverviewMetrics(c *gin.Context) {
	orgID, _ := strconv.ParseUint(c.Query("org_id"), 10, 32)
	clusterID, _ := strconv.ParseInt(c.Query("cluster_id"), 10, 64)

	req := &pbtl.GetOverviewMetricsRequest{
		OrganizationId: uint32(orgID),
		ClusterId:      clusterID,
	}

	resp, err := a.getOverviewMetrics(c.Request.Context(), req)
	if err != nil {
		utils.RespondError(c, 500, "GET_OVERVIEW_METRICS_ERROR", err.Error())
		return
	}
...
func (a *AnalyticsController) getOverviewMetrics(ctx context.Context, req *pbtl.GetOverviewMetricsRequest) (*pbtl.OverviewMetrics, error) {
	log.Debug().
		Uint32("org_id", req.OrganizationId).
		Int64("cluster_id", req.ClusterId).
...
// GetClusterConfig retrieves cluster configuration (placeholder)
// GET /api/analytics/cluster-config?org_id=1&cluster_id=1
func (a *AnalyticsController) GetClusterConfig(c *gin.Context) {
	// This is a placeholder implementation from the original analytics service
	utils.RespondOK(c, &pbtl.ClusterConfigResponse{
		ClusterScalingMode:   "auto",
		EnableHpa:            true,
		EnableVpa:            true,
...
// ClusterMetricsResponse defines the response structure for cluster metrics
...
// GetMetrics streams cluster metrics via SSE
// GET /api/organizations/:org_id/clusters/analytics/:cluster_id/metrics
func (a *AnalyticsController) GetMetrics(c *gin.Context) {
	...
	if err != nil {
		utils.RespondError(c, 400, "BAD_INPUT", "Invalid cluster_id")
		return
	}
	...
	if err := models.GetDB().Where("id = ? AND organization_id = ?", clusterID, orgID).First(&cluster).Error; err != nil {
		utils.RespondError(c, 404, "BAD_INPUT", "Cluster not found in this organization")
		return
	}
	...
	sendOverviewData := func() {
		overviewResp, err := a.getOverviewMetrics(c.Request.Context(), &pbtl.GetOverviewMetricsRequest{
			OrganizationId: uint32(orgID),
			ClusterId:      int64(clusterID),
		})

		if err != nil {
			log.Error().Err(err).Msg("Failed to get overview metrics")
			return
		}
...
// GetSLOMetricsSSE streams SLO/service metrics via SSE
// GET /api/organizations/:org_id/clusters/analytics/:cluster_id/slo-metrics
func (a *AnalyticsController) GetSLOMetricsSSE(c *gin.Context) {
	orgID, err := parseOrgID(c)
	if err != nil {
		return
	}
	...
	sendServicesData := func() {
		servicesResp, err := a.getServices(c.Request.Context(), &pbtl.GetServicesByClusterIdRequest{
			OrganizationId: uint32(orgID),
			ClusterId:      int64(clusterID),
		})
...
// GetNodesSSE streams node data via SSE
// GET /api/organizations/:org_id/clusters/analytics/:cluster_id/nodes/stream
func (a *AnalyticsController) GetNodesSSE(c *gin.Context) {
	orgID, err := parseOrgID(c)
	if err != nil {
		return
	}
	...
	sendNodesData := func() {
		nodesResp, err := a.getNodes(c.Request.Context(), &pbtl.GetNodesByClusterIdRequest{
			OrganizationId: uint32(orgID),
			ClusterId:      int64(clusterID),
		})
		...
		// Placeholder cluster config
		configResp := &pbtl.ClusterConfigResponse{
			ClusterScalingMode:   "auto",
			EnableHpa:            true,
			EnableVpa:            true,
			EnableNodeHandler:    true,
