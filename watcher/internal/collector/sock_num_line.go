...
func (nl *SocketLine) GetAlreadyExistingSockets() {
	...
	logger.Logger().Debug().Msgf("getting already existing sockets for pid %d, fd %d", nl.pid, nl.fd)
	...
	for _, entry := range fdEntries {
		...
		if err != nil {
			logger.Logger().Warn().Err(err).Uint32("pid", nl.pid).
	...
	for _, fd := range fds {
		if fd.SocketInode != "" && nl.fd == fd.Fd {
			...
			logger.Logger().Debug().Any("skInfo", ts).Uint32("pid", nl.pid).
