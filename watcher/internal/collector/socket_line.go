...
func NewSocketLine(ctx context.Context, pid uint32, fd uint64, fetch bool) *SocketLine {
	...
	if fetch {
		...
		if err != nil {
			logger.Logger().Error().Ctx(ctx).Err(err).Msg("getConnectionInfo failed")
		}
	}
	return skLine
}
...
func (nl *SocketLine) getConnectionInfo() error {
	...
	// add to socket line
	// convert to bpf time
	logger.Logger().Debug().Ctx(nl.ctx).Msgf("Adding socket line read from user space %v", skInfo)
	nl.ClearAll() // clear all previous records
	nl.AddValue(uint64(now.UnixNano()), skInfo)
	return nil
}
