...
func NewCRITool(ctx context.Context) (*CRITool, error) {
	...
	for _, endPoint := range runtimeEndpointPaths {
		...
		logger.Logger().Info().Msgf("Connected successfully to CRI using endpoint %s", endPoint)
		break
	}
...
func (ct *CRITool) FilterNamespace(ns string) bool {
	...
	if ct.nsFilterRx.MatchString(ns) {
		logger.Logger().Debug().Msgf("%s filtered with EXCLUDE_NAMESPACES regex %s", ns, ct.nsFilterRx.String())
		return true
	}
...
func (ct *CRITool) FilterNamespaceWithContainerId(id string) bool {
	...
	if err != nil {
		logger.Logger().Error().Err(err).Msgf("Failed to get container status for container %s", id)
		return true
	}
...
func (ct *CRITool) GetPidsRunningOnContainers() (map[uint32]struct{}, error) {
	...
	for _, c := range list {
		if ct.FilterNamespaceWithContainerId(c.Id) {
			logger.Logger().Debug().Msgf("No tracking on ebpf side for container [%s] - [%s]", c.Id, c.Metadata.Name)
			continue
		}
		runningPids, err := ct.getAllRunningProcsInsideContainer(c.Id)
		if err != nil {
			logger.Logger().Error().Err(err).Msgf("Failed to get runnning pids for container [%s]", c.Id)
			continue
		}
		// logger.Logger().Debug().Msgf("running container [%s-%s] has pids %v", c.Metadata.Name, c.Id, runningPids)
...
func (ct *CRITool) getAllRunningProcsInsideContainer(containerID string) ([]uint32, error) {
	...
	if err != nil {
		logger.Logger().Error().Err(err).Msgf("Failed to get container status for container %s", containerID)
		return nil, err
	}
	...
	for _, cgroup := range cgroups {
		if cgroup.HierarchyID == 0 { // cgroup v2
			...
			if err != nil {
				logger.Logger().Warn().Err(err).Msgf("Error reading cgroup.procs file for cgroup %s", cgroup.Path)
				continue
			}
			defer pidFile.Close()
			...
			for fileScanner.Scan() {
				...
				if err != nil {
					logger.Logger().Warn().Err(err).Msgf("Error parsing pid %s", fileScanner.Text())
					continue
				}
