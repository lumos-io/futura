...
func DeployAndWait(parentCtx context.Context, ch chan interface{}) {
	...
	if err != nil {
		logger.Logger().Fatal().Err(err).Msg("link sys_enter_read tracepoint")
	}
	logger.Logger().Info().Msg("sys_enter_read linked")
	defer func() {
		logger.Logger().Info().Msg("closing sys_enter_read tracepoint")
		l.Close()
	}()

	l1, err := link.Tracepoint("syscalls", "sys_enter_write", L7BpfProgsAndMaps.bpfPrograms.SysEnterWrite, nil)
	if err != nil {
		logger.Logger().Warn().Str("verifier log", string(L7BpfProgsAndMaps.bpfPrograms.SysEnterWrite.VerifierLog)).Msg("verifier log")
		logger.Logger().Fatal().Err(err).Msg("link sys_enter_write tracepoint")
	}
	logger.Logger().Info().Msg("sys_enter_write linked")
	defer func() {
		logger.Logger().Info().Msg("closing sys_enter_write tracepoint")
		l1.Close()
	}()

	l2, err := link.Tracepoint("syscalls", "sys_exit_read", L7BpfProgsAndMaps.bpfPrograms.SysExitRead, nil)
	if err != nil {
		logger.Logger().Fatal().Err(err).Msg("link sys_exit_read tracepoint")
	}
	logger.Logger().Info().Msg("sys_exit_read linked")
	defer func() {
		logger.Logger().Info().Msg("closing sys_exit_read tracepoint")
		l2.Close()
	}()

	l3, err := link.Tracepoint("syscalls", "sys_enter_sendto", L7BpfProgsAndMaps.bpfPrograms.SysEnterSendto, nil)
	if err != nil {
		logger.Logger().Fatal().Err(err).Msg("link sys_enter_sendto tracepoint")
	}
	logger.Logger().Info().Msg("sys_enter_sendto linked")
	defer func() {
		logger.Logger().Info().Msg("closing sys_enter_sendto tracepoint")
		l3.Close()
	}()

	l4, err := link.Tracepoint("syscalls", "sys_enter_recvfrom", L7BpfProgsAndMaps.bpfPrograms.SysEnterRecvfrom, nil)
	if err != nil {
		logger.Logger().Fatal().Err(err).Msg("link sys_enter_recvfrom tracepoint")
	}
	logger.Logger().Info().Msg("sys_enter_recvfrom linked")
	defer func() {
		logger.Logger().Info().Msg("closing sys_enter_recvfrom tracepoint")
		l4.Close()
	}()

	l5, err := link.Tracepoint("syscalls", "sys_exit_recvfrom", L7BpfProgsAndMaps.bpfPrograms.SysExitRecvfrom, nil)
	if err != nil {
		logger.Logger().Fatal().Err(err).Msg("link sys_exit_recvfrom tracepoint")
	}
	logger.Logger().Info().Msg("sys_exit_recvfrom linked")
	defer func() {
		logger.Logger().Info().Msg("closing sys_exit_recvfrom tracepoint")
		l5.Close()
	}()

	l6, err := link.Tracepoint("syscalls", "sys_exit_sendto", L7BpfProgsAndMaps.bpfPrograms.SysExitSendto, nil)
	if err != nil {
		logger.Logger().Fatal().Err(err).Msg("link sys_exit_sendto tracepoint")
	}
	logger.Logger().Info().Msg("sys_exit_sendto linked")
	defer func() {
		logger.Logger().Info().Msg("closing sys_exit_sendto tracepoint")
		l6.Close()
	}()

	l7, err := link.Tracepoint("syscalls", "sys_exit_write", L7BpfProgsAndMaps.bpfPrograms.SysExitWrite, nil)
	if err != nil {
		logger.Logger().Fatal().Err(err).Msg("link sys_exit_write tracepoint")
	}
	logger.Logger().Info().Msg("sys_exit_write linked")
	defer func() {
		logger.Logger().Info().Msg("closing sys_exit_write tracepoint")
		l7.Close()
	}()
	...
	defer func() {
		logger.Logger().Info().Msg("closing l7 events perf event array reader")
		l7Events.Close()
	}()
	...
	go func() {
		...
		read := func() {
			...
			if record.LostSamples != 0 {
				logger.Logger().Warn().Msgf("lost #%d samples due to ring buffer's full", record.LostSamples)
			}

			if record.RawSample == nil || len(record.RawSample) == 0 {
				logger.Logger().Warn().Msgf("read empty record from perf array")
				return
			}
			...
			parseLogMessage := func(input []byte, logMsg *bpfLogMessage) []byte {
				...
				if len(parts) != 2 {
					logger.Logger().Warn().Msgf("invalid ebpf log message: %s", string(input))
					return nil
				}

				parsedArgs := bytes.SplitN(parts[1], []byte("|"), 3)
				if len(parsedArgs) != 3 {
					logger.Logger().Warn().Msgf("invalid ebpf log message not 3 args: %s", string(input))
					return nil
				}
			...
			if logMessage == nil {
				logger.Logger().Warn().Msgf("invalid ebpf log message: %s", string(logMsg.LogMsg[:]))
				return
			}

			switch logMsg.Level {
			case 0:
				logger.Logger().Debug().Str("func", string(funcName)).Uint32("pid", logMsg.Pid).
			...
			case 1:
				logger.Logger().Info().Str("func", string(funcName)).Uint32("pid", logMsg.Pid).
					Uint64(args[0].argName, args[0].argValue).Uint64(args[1].argName, args[1].argValue).Uint64(args[2].argName, args[2].argValue).
					Str("log-msg", string(logMessage)).Msg("ebpf-log")
			case 2:
				logger.Logger().Warn().Str("func", string(funcName)).Uint32("pid", logMsg.Pid).
					Uint64(args[0].argName, args[0].argValue).Uint64(args[1].argName, args[1].argValue).Uint64(args[2].argName, args[2].argValue).
					Str("log-msg", string(logMessage)).Msg("ebpf-log")
			case 3:
				logger.Logger().Error().Str("func", string(funcName)).Uint32("pid", logMsg.Pid).
					Uint64(args[0].argName, args[0].argValue).Uint64(args[1].argName, args[1].argValue).Uint64(args[2].argName, args[2].argValue).
					Str("log-msg", string(logMessage)).Msg("ebpf-log")
			}
		}
	...
	go func() {
		...
		read := func() {
			...
			if record.LostSamples != 0 {
				logger.Logger().Warn().Msgf("lost samples l7-event %d", record.LostSamples)
			}

			// TODO: investigate why this is happening
			if record.RawSample == nil || len(record.RawSample) == 0 {
				logger.Logger().Warn().Msgf("read sample l7-event nil or empty")
				return
			}
