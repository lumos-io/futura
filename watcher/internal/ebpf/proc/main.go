...
func DeployAndWait(parentCtx context.Context, ch chan interface{}) {
	...
	if err != nil {
		logger.Logger().Fatal().Err(err).Msg("link sched_process_exit tracepoint")
	}
	defer func() {
		logger.Logger().Info().Msg("closing sched_process_exit tracepoint")
		l.Close()
	}()

	l1, err := link.Tracepoint("sched", "sched_process_exec", objs.bpfPrograms.SchedProcessExec, nil)
	if err != nil {
		logger.Logger().Fatal().Err(err).Msg("link sched_process_exec tracepoint")
	}
	defer func() {
		logger.Logger().Info().Msg("closing sched_process_exec tracepoint")
		l1.Close()
	}()
	...
	defer func() {
		logger.Logger().Info().Msg("closing pExitEvents ringbuf reader")
		pEvents.Close()
	}()
	...
	go func() {
		for {
			read := func() {
				...
				if err != nil {
					logger.Logger().Warn().Err(err).Msg("error reading from pExitEvents")
				}

				if record.RawSample == nil || len(record.RawSample) == 0 {
					logger.Logger().Debug().Msgf("read sample l7-event nil or empty")
					return
				}
