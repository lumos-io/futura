...
func TestParseSSLLib(t *testing.T) {
	...
	if lib.version != "v1.1" {
		t.Fatalf("expected version v1.1, got %s", lib.version)
	}

	lib = libs["/usr/lib64/libssl3.so"]
	if lib.version != "v3" {
		t.Fatalf("expected version v3, got %s", lib.version)
	}

	lib = libs["/usr/lib64/libssl.so.1.0.2k"]
	if lib.version != "v1.0.2" {
		t.Fatalf("expected version v1.0.2, got %s", lib.version)
	}
